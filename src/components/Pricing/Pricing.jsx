    ...
    return (
     <section className="py-20 px-4">
            ...
            <div className='flex flex-col items-center space-y-3 text-center mb-16'>
                ...
                <p className='text-gray-500 max-w-md'>Choose the plan that fits your needs. Upgrade or downgrade anytime.</p>
            </div>
                ...
                {plans.map((item) => (
                    ...
                    >
                        ...
                        <div className="space-y-2 mb-8">
                            ...
                            <p className={`text-sm font-medium ${item.isPopular ? "text-purple-100" : "text-slate-500"}`}>
                                {item.description}
                            </p>
                        </div>
                        ...
                        <div className="mb-10 flex items-baseline gap-1">
                            ...
                            <span className={`text-sm font-bold ${item.isPopular ? "text-purple-200" : "text-slate-400"}`}>
                                /{item.period}
                            </span>
                        </div>
                        ...
                        <ul className="space-y-4 mb-10 flex-grow">
                            {item.features.map((feature, index) => (
                                <li key={index} className="flex items-center gap-3 text-sm font-semibold">
                                    <div className={`p-1 rounded-full ${item.isPopular ? " text-white" : " text-purple-600"}`}>
                                        <Check size={14} strokeWidth={4} />
                                    </div>
                                    <span className={item.isPopular ? "text-purple-50" : "text-slate-600"}>{feature}</span>
                                </li>
                            ))}
                        </ul>
