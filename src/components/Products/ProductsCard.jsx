...
if (!isBuy) {
  ...
  <div className="flex justify-between items-start mb-6">
    ...
    <span className={`text-xs font-bold px-4 py-1.5 rounded-full ${tagStyles[products.tag]}`}>
      {
        products.tag
      }
    </span>
  ...
  <div className="mt-6 flex items-baseline gap-1">
    <span className="text-3xl font-extrabold text-slate-800">${products.price}</span>
    <span className="text-slate-400 text-sm">/{
        products.period}</span>
  </div>
  ...
  <div className="mt-8">
    <button
    onClick={ handelBuy} disabled={isBuy}
    className= {`
